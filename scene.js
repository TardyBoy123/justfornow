...
(() => {
 ...
 function line(points,alpha=.35,width=1){ctx.strokeStyle=`rgba(87,69,127,${alpha})`;ctx.lineWidth=width;ctx.beginPath();points.forEach((p,i)=>i?ctx.lineTo(...p):ctx.moveTo(...p));ctx.stroke();}
 function draw(){
  ...
  for(let k=0;k<3;k++){
   ...
   ctx.strokeStyle=`rgba(87,69,127,${k===0?.43:.22})`;ctx.lineWidth=k===0?1.2:.7;
   ...
   ctx.fillStyle='#685584';ctx.beginPath();ctx.arc(x,y,k===0?4:2,0,Math.PI*2);ctx.fill();ctx.restore();
  }
  ...
  for(let i=1;i<=4;i++){ctx.strokeStyle='#8c7caa';ctx.lineWidth=.8;ctx.beginPath();ctx.ellipse(bx,by,rr*i*.5,rr*i*.13,-.2,0,Math.PI*2);ctx.stroke();}
  line([[bx-rr*.4,by],[bx,by-rr*1.4],[bx+rr*.4,by]],.5,.8);
  ctx.restore();
  ctx.font='9px "Courier New",monospace';ctx.fillStyle='#837494';ctx.fillText('JFN / ORBIT '+(p+1).toFixed(2),Math.max(20,cx-r),cy+r+25);
 }
